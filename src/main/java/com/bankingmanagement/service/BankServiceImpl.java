...
public class BankServiceImpl implements BankService{
    ...
    public BankTO addBank(BankRequest bank) throws BankDetailsNotFoundException {
        ...
        Set<Branch> branches = bank.getBranchList().stream()
                .map(branchTO -> {
                    Branch branch = new Branch();
                    branch.setBranchName(branchTO.getBranchName());
                    branch.setBranchAddress(branchTO.getBranchAddress());
                    branch.setBank(newBank); // ✅ Set the bank for foreign key mapping
                    return branch;
    ...
    @Override
    public BankTO updateBank(int bankCode, BankRequest bankRequest) throws BankDetailsNotFoundException {
        log.info("BankServiceImpl.updateBank: Updating bank with code: {}", bankCode);

        Bank existingBank = bankRepository.findById(bankCode)
                .orElseThrow(() -> new BankDetailsNotFoundException("Bank not found with code: " + bankCode));

        existingBank.setBankName(bankRequest.getBankName());
        existingBank.setBankAddress(bankRequest.getBankAddress());

        if (bankRequest.getBranchList() != null && !bankRequest.getBranchList().isEmpty()) {
            Set<Branch> updatedBranches = bankRequest.getBranchList().stream().map(branchReq -> {
                Branch branch = new Branch();
                branch.setBranchName(branchReq.getBranchName());
                branch.setBranchAddress(branchReq.getBranchAddress());
                branch.setBank(existingBank);
                return branch;
            }).collect(Collectors.toSet());

            existingBank.setBranchSet(updatedBranches);
        }

        Bank updatedBank = bankRepository.save(existingBank);

        return new BankTO(
                updatedBank.getBankCode(),
                updatedBank.getBankName(),
                updatedBank.getBankAddress(),
                updatedBank.getBranchSet().stream()
                        .map(branch -> new BranchTO(
                        branch.getBranchId(),
                        branch.getBranchName(),
                        branch.getBranchAddress(),
